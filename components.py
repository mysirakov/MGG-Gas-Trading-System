...
def stat_card_with_delta(icon: str, label: str, value: str, delta: float = None, delta_label: str = "", color: str = "blue", inverse_colors: bool = False):
    ...
    if delta is not None:
        ...
        delta_html = f'<div class="stat-delta {delta_class}"><span class="material-icons-round">{arrow}</span><span>{abs(delta):.1f}%</span><span class="delta-label">{delta_label}</span></div>'
    
    html = f'<div class="stat-card-delta"><div class="stat-icon {color}"><span class="material-icons-round">{icon}</span></div><div class="stat-content"><p class="stat-label">{label}</p><p class="stat-value">{value}</p>{delta_html}</div></div>'
